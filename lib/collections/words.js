    ...
    return {
        ...
        updateWords: function(doc, localWords, user) {
          ...
          localWords.forEach(function(word) {
            ...
            if (isInList(user._id, word.highlightsMethod)) {
              logger.trace("User " + user._id + " highlighted method word in localWords: " + word.content);
              Words.update({_id: word._id},{$addToSet: {highlightsMethod: user._id}})
            }
