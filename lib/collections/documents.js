...
Document = function(docPackage) {

    this.extID = docPackage.extID;

    // this.title = fileName;
    this.title = docPackage.title;
...
DocumentManager = (function() {
    return {
        createDocument: function(docPackage) {
            /******************************************************************
             * Create a document --insert a document into the package, don't need to make a copy
             * @params
             *    docPackage - a JSON object with fields "docID" (str ID for matching with external DBs),
             *                                           "docTitle" (str name) and
           ...
           var doc = new Document(docPackage);
        ...
        sampleDocument_test: function(userID) {
            /******************************************************************
             * the original function just sample the document that has least 
             * sampling number(and not assigned to the turker before)
             * Change the loop from indices.length to indices.length - 1
             *    userID - the id of the user we want to serve
             *****************************************************************/
            logger.debug("Sampling a document...");
            var documents = Documents.find().fetch();
            ...
            for (i=0; i<indices.length-1; i++) {
                ...
                logger.trace("This is " + JSON.stringify(sampledDoc));
                logger.trace("Sampled " + sampledDoc.title + " with " +
                    // sampledDoc.annotatedBy.length + " annotations");
                    sampledDoc.sampledBy.length + " samples");
                if (!isInList(userID, sampledDoc.annotatedBy)) {
            ...
            logger.trace ("Pringing test msg:" + sampledDoc.sentences);
            DocumentManager.markSampledBy(sampledDoc, MyUsers.findOne(userID));
